 ...
 * List of Searches
 */
exports.list = function(req, res) { Search.find().sort('-created').populate('user', 'displayName').exec(function(err, searches) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
 ...
 * Search middleware
 */
exports.searchByID = function(req, res, next, id) { Search.findById(id).populate('user', 'displayName').exec(function(err, search) {
