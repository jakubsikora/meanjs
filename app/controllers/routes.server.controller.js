 ...
 * List of Routes
 */
exports.list = function(req, res) { Route.find().sort('-created').populate('user', 'displayName').exec(function(err, routes) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
 ...
 * Route middleware
 */
exports.routeByID = function(req, res, next, id) { Route.findById(id).populate('user', 'displayName').exec(function(err, route) {
